...
func main() {
	...
	for i := 0; i < 300; i++ {
		for j := 0; j < 300; j++ {
			testRes := calculateTrajectory(i, j, xFrom, xTo, yFrom, yTo)
			score = max(score, testRes)
		}
	}
...
func calculateTrajectory(xVelocity, yVelocity int, xFrom, xTo, yFrom, yTo int) int {
