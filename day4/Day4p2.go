	...
	for _, n := range numbers {
		...
		for boardId, rowcol := range affectedBoards {
			...
			if boardRes[row] == boardSize || boardRes[boardSize+col] == boardSize {
				gameState.winners[boardId] = true
				gameState.winnersCount++
				lastWinner = boardId
			}
		}
		if gameState.winnersCount == len(gameState.boards) {
			break
		}
	}
	println("Winner board id: " + strconv.Itoa(lastWinner))

	score := calculateScore(gameState.boards[lastWinner], calledNumbers)
	println(score)
}
